// Script to insert new destinations into destinations.ts
const fs = require('fs');
...
const newDestinations = [
  ...
  { id: 'kilimanjaro', city: 'Kilimanjaro', country: 'Tanzania', airportCode: 'JRO', region: 'africa', vibes: ['adventure', 'nature'], bestMonths: [1, 2, 7, 8, 9], averageCost: 3500, highlights: ['Summit Africa highest peak', 'Trekking routes', 'Unique ecosystems', 'Achievement of a lifetime'], latitude: -3.0674, longitude: 37.3556 },
  ...
  // EUROPE
  { id: 'bergamo', city: 'Bergamo', country: 'Italy', airportCode: 'BGY', region: 'europe', vibes: ['culture', 'history', 'food'], bestMonths: [4, 5, 6, 9, 10], averageCost: 1800, highlights: ['Citta Alta', 'Venetian walls', 'Lombardy cuisine', 'Day trips to Milan'], latitude: 45.6983, longitude: 9.6773 },
...
const destPath = path.join(__dirname, '../lib/flash/destinations.ts');
let content = fs.readFileSync(destPath, 'utf-8');

// Generate the new destination entries
let newContent = '\n  // ============================================\n';
newContent += '  // NEW DESTINATIONS (Jan 2026 expansion to 500)\n';
newContent += '  // ============================================\n';

for (const d of newDestinations) {
  newContent += `  {\n`;
  newContent += `    id: '${d.id}',\n`;
  newContent += `    city: '${d.city}',\n`;
  newContent += `    country: '${d.country}',\n`;
  newContent += `    airportCode: '${d.airportCode}',\n`;
  newContent += `    region: '${d.region}',\n`;
  newContent += `    vibes: [${d.vibes.map(v => `'${v}'`).join(', ')}],\n`;
  newContent += `    bestMonths: [${d.bestMonths.join(', ')}],\n`;
  newContent += `    averageCost: ${d.averageCost},\n`;
  newContent += `    highlights: [${d.highlights.map(h => `'${h.replace(/'/g, "\\'")}'`).join(', ')}],\n`;
  newContent += `    imageUrl: 'https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?w=800',\n`;
  newContent += `    latitude: ${d.latitude},\n`;
  newContent += `    longitude: ${d.longitude},\n`;
  newContent += `  },\n`;
}

// Find the closing bracket of the array and insert before it
content = content.replace(/\n\];\s*\n\n\/\/ Helper functions/, newContent + '];\n\n// Helper functions');

// Write back
fs.writeFileSync(destPath, content);

console.log(`Added ${newDestinations.length} new destinations`);
console.log('Total destinations should now be 500');
